...
*NEATBird sorts connection genes by innovation ID. Matching columns describe the same endpoints, so crossover can pick either parent's weight. A gene owned by only one parent is copied without shifting the later matches.*
...
## The bird practiced one level and sat another exam
...
## I generated an impossible Touhou pattern
...
The bird had to move roughly 298 pixels from a low center into a 133-pixel gap. The sprite is 48 pixels tall, leaving about 85 pixels of vertical slack. Flappy Bird movement is not continuous control. A flap resets vertical velocity, and frame updates quantize the resulting altitude. The tested controllers could undershoot or overshoot; I found no clean trajectory through the opening.

I had generated a transition that was undodgeable in the tested game physics and then spent generations asking the agent to get good.
...
I tuned the constants by playing candidate schedules through the real Pygame physics with a heuristic controller. Each sampled transition had to remain passable without becoming easy.
...
It had eight nodes and eight connection genes, seven of them enabled. Inside training, it cleared all 39 pipes available before the frame cap. In a longer replay, it reached 66 pipes and died at frame 4,972.
...
The 3,000-frame run cleared 39 pipes during training. Outcomes after frame 3,000 could not affect its gradient updates. The champion continued through another 27 pipes before the same horizon problem returned at a larger scale.

The obvious cheap option is another run with a larger `--max-frames`. The structural option is replacing the fixed-length `lax.scan` with a bounded `while_loop` that ends when every bird is dead or a score cap is reached. That would stop wasting work on frozen dead lanes and make the simulated episode end condition explicit.

It would also require replacing `scan`'s automatically stacked trajectory outputs with explicit fixed-size buffers and masks in the loop carry, plus a real rewrite of the vectorized engine. “Just use a while loop” is one of those sentences that becomes significantly less charming after opening the profiler.
...
The champion cleared all 39 pipes inside the training window and another 27 beyond it before dying at frame 4,972.
