...
and penalized networks for excess connections. This is a Ha-style diversity
heuristic, not canonical NEAT speciation with explicit fitness sharing. In Ha's
own analogy, deep learning researchers were pop idols while
...
I took the longer route: building the genome format, innovation tracking, graph mutations, crossover, clustering, feed-forward compiler, policy-gradient optimizer, checkpoint format, vectorized game engine, and SVG renderer myself. Well, I actually just asked Claude. The final trainer combines [NEAT-style evolution](https://nn.cs.utexas.edu/downloads/papers/stanley.ec02.pdf) with REINFORCE updates in JAX. Python evolves the graph. JAX trains its weights and simulates the flock. Pygame remains around for replay because I am still not fluent in clanker (all float values).
...
The candidate practiced on one pipe layout, then rollback and evolutionary
selection scored it on three different layouts. These were not held-out test
seeds: update acceptance, fitness, clustering, archives, and champion selection
all depended on them. They were optimization data, but the two optimization
signals disagreed.
...
That was strong evidence that the transition was pathological, not proof that
no action sequence could clear it. I did not run an exhaustive reachability
search in either the Pygame mask engine or the JAX AABB engine.
...
less pathological. It is an engineering heuristic, not an exhaustive proof
that every generated transition is reachable.
...
Pipes 40 onward are an unseen suffix of the seed-0 schedule. The 66-pipe replay
is temporal extrapolation on that schedule, not cross-schedule generalization.
...
| Median pipes | 13.0 |
| P10 | 0.0 |
| P90 | 60.300000000000026 |
| Minimum | 0 |
| Survived 5,000 frames | 0.08 |
...
The plateau had three causes. Gradient updates practiced one layout while
selection and rollback used others. The fixed horizon hid the bird's eventual
death. The schedule heuristic allowed an observed transition that was
pathological for the tested controllers.
...
- JAX batches matching graphs and runs training and held-out episodes;
- Pygame provides the replay state machine and pixel-mask collisions;
- deterministic schedules make failures reproducible;
- portable JSON checkpoints let every strange bird be inspected later.

The champion cleared all 39 pipes inside its seed-0 optimization window and
another 27 on the unseen suffix of that schedule before dying at frame 4,972.
Across 100 held-out schedules, it averaged 20.1 pipes and survived the
5,000-frame cap on 8 percent of schedules.
