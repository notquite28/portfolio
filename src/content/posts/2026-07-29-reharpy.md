...
description: "How I brought Harpy back by making Twitter's website impersonate the API it killed."
...
## The bird is dead. And we have killed it.

It is 2026 and enshittification is the name of the game. Twitter has been renamed to a single letter, tweets are posts, verified badges are a subscription product, and the official API costs somewhere between a mortgage and a hostage negotiation.

Opening the official Twitter app (still calling it Twitter) feels like reading a group chat while Grok-generated slop, community-note ragebait matches under clearly inflammatory posts, and subscription peddling fight for the screen.

Then there was the original [**Harpy app**](https://github.com/robertodoering/harpy): a beautiful Flutter client with timelines, profiles, themes, filters, gestures, and the kind of polish that only happens when one person cares about UX. One issue: it depended on an API that had been taken outside and shot. Still, it felt like finding an unmodded Nissan Skyline GT-R R33 in a world where every car was now an electric SUV: old, impractical, and much more fun than the replacement.

My Discord friend [**Draff**](https://github.com/ItsLogic/) decided to revive it on a whim. His vaguely relevant experience was whipping GLM to enable all chapters in the MangaPlus Tachiyomi extension until it worked. Mine was React. Not React Native, actual web React. Obviously, this was a flawless foundation for reviving a dead Flutter client.

The project is now called [**reHarpy**](https://github.com/ItsLogic/reHarpy) (possibly still private). Draff discovered and documented Twitter's internal APIs. I stripped the app down to one Android APK: no free/pro tier, App Store upsell, or paywall protecting revenue for a business that no longer exists.
...
One class lies convincingly enough that the rest of Harpy never notices. The bridge translates the old v1.1-shaped calls into Twitter's web GraphQL and internal endpoints, then reshapes the responses before they reach the app.
...
Teaching every Harpy feature about Twitter GraphQL would turn the application into forty unrelated reverse-engineering experiments wearing the same theme, which is stupid.
...
Harpy feature
  ...
  -> old v1.1-shaped URL
  -> WebTwitterClient
  -> Twitter web GraphQL or internal endpoint
  -> v1-compatible JSON
...
the bridge routes it to Twitter's `HomeTimeline` operation.
...
## The horse has a browser inside.

The official version of Harpy used Twitter OAuth. In 2023, the lowest reported Enterprise API tier cost [$42,000 per month](https://www.wired.com/story/twitter-data-api-prices-out-nearly-everyone/). I'd rather burn that money on a Toyota GR86. reHarpy does not add consumer keys, developer portals, password login, or a mysterious textbox asking users to paste credentials into an APK from GitHub.

Authentication happens in a WebView.

The user signs into Twitter through the real website. reHarpy captures the authenticated browser session after both required cookies exist:

- `auth_token`
- `ct0`

It also keeps the surrounding browser cookie jar. This matters because a modern authenticated Twitter session is not simply two strings wearing a trench coat. The browser accumulates cookies for identity, Cloudflare, personalization, and other session state that can affect whether a request looks legitimate.
...
### Twitter.com is the API documentation

Calling Twitter's web endpoints is easy. Keeping them working is a FromSoftware side quest: one dependency moved to another bundle, a previously optional header is now mandatory, and the only documentation is a dead account saying "works for me."
...
Each operation also has a query ID embedded in Twitter's current JavaScript bundle. Those IDs rotate. Hard-coding the ID observed on Tuesday is a good way to make the app break shortly after someone writes "works perfectly" in the release notes.
...
Current Twitter web requests can include an `x-client-transaction-id` generated from live browser state: DOM state, animations and computed CSS, Web Crypto, WebRTC, and Twitter's JavaScript modules. We had a pure-Dart imitation. We deleted it. At some point, "implement a browser inside Dart" stops being a clever optimization and becomes the part where the Evangelion runs out of budget.

Android now uses an authenticated headless WebView. The command-line tools use an isolated Chromium CDP session. The runtime finds the current transaction generator inside Twitter's own bundle and produces a fresh value for the exact HTTP method and request path.


### A good browser trace can still lie to you
...
On Android, reHarpy prefers Cronet for its Chrome-like TLS and HTTP/2 stack. Desktop tests use Dart's deterministic `http.Client`, which is less authoritative for answering, "Will Twitter think this came from a browser-shaped Android application?"
...
My first guess was query-ID rotation, because the boring answer is usually right. It was wrong. Twitter had started exposing two initialized transaction-generator candidates in the bundle: one stale, one current. The runtime found both and selected the stale one, producing transaction IDs that looked valid but were no longer accepted.

Selecting the newest live candidate restored writes.

Neither of us knows why Twitter shipped both. I have decided that is not my question to answer.
...
This is deliberate. “Maybe Twitter will accept a write that we know is malformed” is not a recovery strategy.
...
Twitter returned the created tweet under:
...
This is why the reshaper exists as one explicit compatibility seam. Twitter can move a result node without requiring the compose screen, timeline state, and tweet widgets to develop opinions about GraphQL archaeology.

### The quote's True Name was a URL

The first quote smoke produced a normal post with text but no quoted post.

The bridge was sending `quote_tweet_id`. Twitter's current composer expected the original status URL through `attachment_url`.
...
Twitter serves JSON as `application/json` with no `charset`. Dart's `response.body` sees no encoding, falls back to Latin-1, and every Japanese post looks like a Vocaloid voicebank opened in Notepad. The fix is to read bytes and decode them properly:
...
Mapped writes are more conservative. They may recover once after HTTP `403`/`404` or recognized automation responses such as Twitter codes `226`, `334`, and `344`.
...
If a create request times out after reaching Twitter, the client cannot know whether the post exists. Retrying might create a duplicate. The honest response is an ambiguous failure, not optimism with side effects.
...
Media upload now uses Twitter's chunked web endpoint, converting selected videos to H.264/AAC first. View counts, Grok translations, custom share domains, system sharing, and image export also crossed the facade without making the old widgets understand Twitter's current response shapes.
...
## Equivalent exchange, more or less

reHarpy is the real-world equivalent of microwaving a banana and hoping we discover time travel.
...
Media upload remains sensitive to file validity, transport fingerprinting, and rate limits; alt text and subtitle metadata are not implemented. Cookie storage still flattens browser scope, desktop transport does not perfectly represent Android, and Twitter-side bot detection can reject even correct query IDs and transaction IDs.

reHarpy is production-ready under the traditional AUR definition: the maintainer used it successfully yesterday.

## The bird is dead. Long live Harpy.
