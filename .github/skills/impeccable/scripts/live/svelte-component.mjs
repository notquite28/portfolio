 ...
 * The browser mounts them through a project-major-aware Svelte 4/5 runtime;
 * accept inlines the chosen variant with props mapped to original bindings.
 */
...
const NON_EXPRESSION_MUSTACHE_PREFIXES = new Set(['#', ':', '/', '@']);
...
export function detectInstalledSvelteMajor(cwd = process.cwd()) {
  const packageFile = resolveInstalledSveltePackage(cwd);
  if (!packageFile) {
    throw new Error(`Unable to locate the installed Svelte package from ${cwd}`);
  }
  const version = JSON.parse(fs.readFileSync(packageFile, 'utf-8')).version;
  const major = Number.parseInt(String(version || '').split('.')[0], 10);
  if (major !== 4 && major !== 5) {
    throw new Error(`Unsupported installed Svelte version ${version || 'unknown'}; expected major 4 or 5`);
  }
  return major;
}

function resolveInstalledSveltePackage(cwd) {
  let dir = path.resolve(cwd);
  while (true) {
    const candidate = path.join(dir, 'node_modules', 'svelte', 'package.json');
  ...
  try {
    ...
    return requireFromProject.resolve('svelte/package.json');
  } catch {
    return null;
  }
}

export function ensureRuntimeHelper(cwd = process.cwd(), svelteMajor = detectInstalledSvelteMajor(cwd)) {
  const file = path.join(cwd, SVELTE_RUNTIME_FILE);
  const source = svelteMajor === 5
...
/**
 * Extract ordered unique interpolation and binding expressions from Svelte markup.
 * Block, branch, closing, special and spread mustaches are intentionally excluded.
 */
export function extractMustacheExpressions(text) {
  const expressions = [];
  const seen = new Set();
  for (const token of extractMustacheBindings(text)) {
    if (seen.has(token.expr)) continue;
    seen.add(token.expr);
    expressions.push(token.expr);
  }
  return expressions;
}

export function extractMustacheBindings(text) {
  ...
  while (index < source.length) {
    ...
    if (end < 0) break;
    const expr = source.slice(index + 1, end).trim();
    if (isBindableMustacheExpression(expr)) {
      occurrences.push({
        expr,
        placeholder: source.slice(index, end + 1),
        binding: classifyMustacheBinding(source, index),
      });
    }
    index = end + 1;
  }
  ...
  for (const occurrence of occurrences) {
    const list = groups.get(occurrence.binding.contextKey) || [];
    list.push(occurrence);
    groups.set(occurrence.binding.contextKey, list);
  }
  for (const list of groups.values()) {
    const tokenCount = list.length;
    list.forEach((occurrence, tokenIndex) => {
      occurrence.binding.tokenIndex = tokenIndex;
      occurrence.binding.tokenCount = tokenCount;
      occurrence.binding.ambiguous = tokenCount > 1;
      delete occurrence.binding.contextKey;
    });
  }
  return occurrences;
}

function findMustacheEnd(source, start) {
  let depth = 0;
  let quote = '';
...
function isBindableMustacheExpression(expr) {
  if (!expr || NON_EXPRESSION_MUSTACHE_PREFIXES.has(expr[0])) return false;
  if (expr.startsWith('...') || expr.startsWith('/*')) return false;
  return true;
}

function classifyMustacheBinding(source, tokenStart) {
  const tagStart = source.lastIndexOf('<', tokenStart);
  const tagEnd = source.lastIndexOf('>', tokenStart);
  if (tagStart <= tagEnd || source[tagStart + 1] === '/') {
    const textStart = tagEnd + 1;
    const textEnd = source.indexOf('<', tokenStart);
    return {
      kind: 'text',
      contextKey: `text:${textStart}:${textEnd < 0 ? source.length : textEnd}`,
    };
  }

  const tagPrefix = source.slice(tagStart + 1, tokenStart);
  const attrMatch = tagPrefix.match(/(?:^|\s)([^\s"'=<>`/]+)\s*=\s*(?:"[^"]*|'[^']*|[^\s"'=<>`]*)$/);
  const shorthand = !attrMatch && /^\s*[A-Za-z_$][\w$]*\s*$/.test(
    source.slice(tokenStart + 1, findMustacheEnd(source, tokenStart)),
  );
  const name = attrMatch?.[1] || (shorthand
    ? source.slice(tokenStart + 1, findMustacheEnd(source, tokenStart)).trim()
    : '');
  const kind = name.includes(':') || name.includes('|') ? 'directive' : 'attribute';
  return {
    kind,
    name,
    contextKey: `${kind}:${tagStart}:${name}`,
  };
}
...
export function buildPropContract(expressions) {
  ...
  for (const item of expressions || []) {
    ...
    if (!expr) continue;
    let entry = byExpression.get(expr);
    if (!entry) {
      entry = { expr, placeholder: `{${expr}}`, bindings: [] };
      byExpression.set(expr, entry);
    }
    if (typeof item === 'object' && item.binding) entry.bindings.push(item.binding);
  }

  const entries = [...byExpression.values()];
  const bases = entries.map((entry) => derivePropName(entry.expr));
  ...
  return entries.map((entry, index) => {
    ...
    const preferred = baseCounts.get(base) > 1 ? `${base}_${stableExprId(entry.expr)}` : base;
    let prop = preferred;
    let collision = 2;
    while (used.has(prop)) prop = `${preferred}_${collision++}`;
    used.add(prop);
    return { prop, ...entry };
  });
}

function derivePropName(expr) {
  if (isSafePropIdentifier(expr)) return expr;
  const tail = expr.match(/(?:\.\s*([A-Za-z_$][\w$]*)|\[\s*['"]([A-Za-z_$][\w$]*)['"]\s*\])\s*$/);
  const leaf = tail?.[1] || tail?.[2];
  return isSafePropIdentifier(leaf) ? leaf : `prop_${stableExprId(expr)}`;
}

function isSafePropIdentifier(value) {
  return typeof value === 'string'
    && /^[A-Za-z_$][\w$]*$/.test(value)
    && !value.startsWith('$')
    && !RESERVED_PROP_NAMES.has(value);
}

function stableExprId(expr) {
  return createHash('sha1').update(expr).digest('hex').slice(0, 8);
}
...
function buildPropsScript(contract, svelteMajor) {
  if (contract.length === 0) return '';
  if (svelteMajor === 4) {
    return `<script>\n${contract.map((c) => `  /** @type {any} */\n  export let ${c.prop};`).join('\n')}\n</script>\n`;
  }
  const names = contract.map((c) => c.prop).join(', ');
  const typeFields = contract.map((c) => `    ${c.prop}: any;`).join('\n');
  return `<script>\n  /** @type {{\n${typeFields}\n  }} */\n  let { ${names} } = $props();\n</script>\n`;
}

function buildVariantStub(variantNum, originalWithProps, contract, svelteMajor) {
  const propsComment = contract.length > 0
    ? `\n<!-- Props: ${contract.map((c) => `${c.prop} <- {${c.expr}}`).join(', ')} -->\n`
    : '';
  return `${buildPropsScript(contract, svelteMajor)}${propsComment}${originalWithProps.trim()}\n\n<style>\n  /* Variant ${variantNum}: add scoped CSS here */\n</style>\n`;
}

function buildInsertVariantStub(variantNum) {
  return `<div class="impeccable-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .impeccable-insert-preview { display: block; }\n</style>\n`;
}
...
export function scaffoldSvelteComponentSession({
  ...
  const svelteMajor = detectInstalledSvelteMajor(cwd);
  ensureRuntimeHelper(cwd, svelteMajor);
  const dir = componentSessionDir(id, cwd);
  fs.mkdirSync(dir, { recursive: true });

  const originalMarkup = originalLines.join('\n');
  const contract = buildPropContract(extractMustacheBindings(originalMarkup));
  ...
  for (let n = 1; n <= count; n++) {
    ...
    if (!fs.existsSync(variantFile)) {
      fs.writeFileSync(variantFile, buildVariantStub(n, originalWithProps, contract, svelteMajor), 'utf-8');
    }
  }
