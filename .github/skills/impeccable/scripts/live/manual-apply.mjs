...
export function createManualApplyController({
  ...
  function pushApplyEventAndWait(batch, pageUrl, chunk = null, repair = null) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        const rollbackGuard = snapshotApplyEventFiles(batch, cwdValue);
        tombstoneTimedOutApplyId(eventId, { batch, rollbackSnapshot, rollbackGuard, pageUrl, cwd: cwdValue });
  ...
  function rollbackTimedOutReply(msg) {
    ...
    if (!details) return { rolledBackFiles: [], rollbackFailures: [], skippedFiles: [] };
  ...
  function cancelPendingEvents(pageUrl, reason = 'manual_edit_discarded') {
    ...
    const shouldNeutralize = (details) => !pageUrl
      || details?.pageUrl === pageUrl
      || details?.batch?.pageUrl === pageUrl;
    ...
    for (const [eventId, deferred] of [...pendingApplyDeferreds.entries()]) {
      ...
      const rollbackGuard = snapshotApplyEventFiles(deferred.batch, cwdValue);
    ...
    for (const [eventId, details] of timedOutApplyIds.entries()) {
      if (!shouldNeutralize(details)) continue;
      const cwdValue = details.cwd || projectCwd();
      timedOutApplyIds.set(eventId, {
        ...details,
        rollbackGuard: snapshotApplyEventFiles(details.batch, cwdValue),
        pageUrl: details.pageUrl || details.batch?.pageUrl || null,
        reason,
        cwd: cwdValue,
      });
      if (!canceledById.has(eventId)) {
        canceledById.set(eventId, {
          id: eventId,
          pageUrl: details.pageUrl || details.batch?.pageUrl || null,
          entryCount: details.batch?.entries?.length || 0,
          rolledBackFiles: [],
          rollbackFailures: [],
        });
      }
...
export function rollbackApplySnapshot(
  ...
  for (const relativeFile of scope) {
    ...
    if (!before) continue;
    const absolute = path.resolve(cwd, relativeFile);
    if (expectedCurrentSnapshot) {
      const expectedCurrent = expectedCurrentSnapshot.get(relativeFile);
      if (!expectedCurrent || !fileMatchesApplySnapshot(absolute, expectedCurrent)) {
        skippedFiles.push({ file: relativeFile, reason: 'content_changed_since_rollback_scheduled' });
        continue;
      }
    }
    try {
  ...
  return { rolledBackFiles, rollbackFailures, skippedFiles };
}

function fileMatchesApplySnapshot(absolute, expected) {
  try {
    const exists = fs.existsSync(absolute);
    if (exists !== expected.exists) return false;
    return !exists || fs.readFileSync(absolute, 'utf-8') === expected.content;
  } catch {
    return false;
  }
}
