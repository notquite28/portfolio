...
/**
 * Regex-on-source checks shared between browser and Node page-level detection.
 * These don't need DOM access; Node may also provide collected stylesheet CSS.
 */
function checkHtmlPatterns(html, collectedCss = '') {
  const findings = [];
  ...
  const gridPatternSource = collectedCss ? `${html}\n${collectedCss}` : html;
  {
    ...
    let blk;
    while ((blk = blockRe.exec(gridPatternSource)) !== null) {
...
function positionedStyleImpliesEscape(style, clipX, clipY) {
  const values = [];
  if (clipX) {
    values.push(
      style.right,
  ...
  const inset = String(style.inset || '').trim();
  if (inset) {
    const tokens = [];
    let token = '';
    let depth = 0;
    for (const char of inset) {
      if (char === '(') depth += 1;
    ...
    if (tokens.length >= 1 && tokens.length <= 4) {
      ...
      if (clipX) values.push(right, left);
      if (clipY) values.push(top, bottom);
    }
  }
...
function checkClippedOverflow(el, style, getStyle) {
  ...
  for (const child of el.querySelectorAll('*')) {
    ...
    if (pos === 'absolute' || pos === 'fixed') {
      ...
      if (escapes === null && !positionedStyleImpliesEscape(childStyle, clipX, clipY)) continue;
