...
Output (JSON):
  ...
  if (args.includes('--remove')) {
    if (svelteKit) {
      const appHtmlResult = updateSvelteKitAppHtml({
        cwd: process.cwd(),
        svelteKit,
        config,
      });
      ...
      console.log(JSON.stringify({
        ok: true,
        adapter: 'sveltekit',
        results: [appHtmlResult, adapterResult],
      }));
      return;
    }
  ...
  if (svelteKit) {
    // Older releases injected live.js directly into app.html. Remove that
    // client before mounting the adapter, then repatch the template CSP for
    // the adapter's dynamically-created localhost script.
    const appHtmlResult = updateSvelteKitAppHtml({
      cwd: process.cwd(),
      svelteKit,
      config,
...
function updateSvelteKitAppHtml({ cwd, svelteKit, config, port }) {
  const relFile = svelteKit.appHtml;
  const absFile = path.resolve(cwd, relFile);
  const content = fs.readFileSync(absFile, 'utf-8');
  const detagged = removeTag(content, config.commentSyntax);
  const restored = revertCspMeta(detagged);
  const updated = port === undefined ? restored : patchCspMeta(restored, port);

  if (updated !== content) fs.writeFileSync(absFile, updated, 'utf-8');

  return {
    file: relFile,
    removed: detagged !== content,
    cspReverted: restored !== detagged,
    ...(port === undefined ? {} : { cspPatched: updated !== restored }),
  };
}
