...
function discoverRootsForPattern(repoRoot, rawPattern) {
  ...
  if (segments.includes('**')) {
    const matchingDirs = [];
    walkDirs(base, (dir) => {
      const relSegments = path.relative(repoRoot, dir).split(path.sep).filter(Boolean);
      if (workspacePatternMatchesSegments(segments, relSegments, false)) matchingDirs.push(dir);
    });
    const packageRoots = matchingDirs.filter(isCandidateProjectRoot);
    if (packageRoots.length) return packageRoots;
    return matchingDirs.filter((dir) => path.dirname(dir) === base);
  }
...
function workspacePatternMatchesRel(pattern, relSegments) {
  ...
  return workspacePatternMatchesSegments(patternSegments, relSegments, true);
}

function workspacePatternMatchesSegments(patternSegments, relSegments, allowRelSuffix) {
  if (!patternSegments.length) return false;
  const failed = new Set();
  function match(patternIndex, relIndex) {
    if (patternIndex === patternSegments.length) {
      return allowRelSuffix || relIndex === relSegments.length;
    }
    const key = `${patternIndex}:${relIndex}`;
    if (failed.has(key)) return false;
    const segment = patternSegments[patternIndex];
    const matched = segment === '**'
      ? match(patternIndex + 1, relIndex)
        || (relIndex < relSegments.length && match(patternIndex, relIndex + 1))
      : relIndex < relSegments.length
        && segmentMatches(segment, relSegments[relIndex])
        && match(patternIndex + 1, relIndex + 1);
...
function projectRootFromDoubleStarPattern(repoRoot, relSegments, patternSegments) {
  if (!workspacePatternMatchesSegments(patternSegments, relSegments, true)) return null;
  const targetDir = path.join(repoRoot, ...relSegments);
  const packageRoot = nearestPackageRootBetween(repoRoot, targetDir, repoRoot);
  if (packageRoot) {
    const packageSegments = path.relative(repoRoot, packageRoot).split(path.sep).filter(Boolean);
    if (workspacePatternMatchesSegments(patternSegments, packageSegments, false)) return packageRoot;
  }

  // Without a package marker, use the shortest matching prefix as the
  // workspace boundary. A terminal ** after a literal directory (the common
  // "packages/**" form) must consume one child segment to name a project.
  let matchLength = 1;
  while (
    matchLength <= relSegments.length
    && !workspacePatternMatchesSegments(patternSegments, relSegments.slice(0, matchLength), false)
  ) {
    matchLength++;
  }
  const lastConcreteIndex = patternSegments.findLastIndex((segment) => segment !== '**');
  if (
    matchLength <= relSegments.length
    && patternSegments.at(-1) === '**'
    && lastConcreteIndex === patternSegments.length - 2
    && !patternSegments[lastConcreteIndex].includes('*')
    && matchLength < relSegments.length
  ) {
    matchLength++;
  }
  return matchLength <= relSegments.length
    ? path.join(repoRoot, ...relSegments.slice(0, matchLength))
    : null;
}
...
function updateCheckDisabledByConfig(projectRoot = process.cwd()) {
  let value;
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(projectRoot, '.impeccable', name), 'utf-8'));
...
async function computeUpdateDirective(projectRoot = process.cwd(), now = Date.now()) {
  try {
    if (process.env.IMPECCABLE_NO_UPDATE_CHECK) return null;
    if (updateCheckDisabledByConfig(projectRoot)) return null;
...
async function cli() {
  ...
  const updateDirective = await computeUpdateDirective(ctx.projectRoot);
