...
(function () {
  ...
  function insertAnchorPathChildren(parent) {
    return [...(parent?.children || [])].filter((child) => {
      if (own(child)) return false;
      if (child.hasAttribute?.('data-impeccable-insert-placeholder')) return false;
      return true;
    });
  }

  function buildInsertAnchorDomPath(anchor) {
    ...
    while (node && node !== document.body) {
      ...
      const index = insertAnchorPathChildren(parent).indexOf(node);
      if (index < 0) return null;
      path.push(index);
      node = parent;
    }
    return node === document.body ? path.reverse() : null;
  }

  function resolveInsertAnchorDomPath(path) {
    if (!Array.isArray(path)) return null;
    let node = document.body;
    for (const index of path) {
      if (!Number.isInteger(index) || index < 0) return null;
      node = insertAnchorPathChildren(node)[index];
      if (!node) return null;
    }
    return node === document.body ? null : node;
  }

  function insertAnchorMatchesSnapshot(candidate, snap) {
    if (!candidate || own(candidate)) return false;
    if (candidate.tagName !== String(snap.anchorTag || 'DIV').toUpperCase()) return false;
    const classes = String(snap.anchorClasses || '').split(/\s+/).filter(Boolean);
    if (!classes.every((name) => candidate.classList.contains(name))) return false;
    const expectedText = normalizePreviewText(snap.anchorText);
    if (expectedText && !normalizePreviewText(candidate.textContent).startsWith(expectedText)) return false;
    return true;
  }

  function matchingInsertAnchors(anchorTag, anchorClasses, anchorText) {
    const snap = { anchorTag, anchorClasses, anchorText };
    return [...document.getElementsByTagName(String(anchorTag || 'div'))]
      .filter((candidate) => insertAnchorMatchesSnapshot(candidate, snap));
  }

  function buildInsertPlaceholderSnapshotFromDom(anchor, placeholder) {
    const anchorTag = anchor.tagName || 'DIV';
    const anchorClasses = normalizeElementClassName(anchor);
    const anchorText = normalizePreviewText(anchor.textContent).slice(0, 120);
    const matchingAnchors = matchingInsertAnchors(anchorTag, anchorClasses, anchorText);
    return {
      ...
      anchorDomPath: buildInsertAnchorDomPath(anchor),
      anchorOrdinal: Math.max(0, matchingAnchors.indexOf(anchor)),
      anchorTag,
      anchorClasses,
      anchorText,
    };
  ...
  function findInsertAnchorInDom() {
    ...
    const byPath = resolveInsertAnchorDomPath(snap.anchorDomPath);
    if (insertAnchorMatchesSnapshot(byPath, snap)) return byPath;

    const candidates = matchingInsertAnchors(snap.anchorTag, snap.anchorClasses, snap.anchorText);
    const ordinal = Number.isInteger(snap.anchorOrdinal) ? snap.anchorOrdinal : -1;
    return ordinal >= 0 ? candidates[ordinal] || null : null;
  }
  ...
  function buildSveltePropValuesFromLiveElement(liveEl, manifest) {
    const contract = manifest?.propContractVersion === 2 ? manifest.propContract || [] : [];
    const values = {};
    ...
    for (const entry of contract) {
      ...
      for (const binding of entry.bindings || []) {
        const value = readSvelteBindingValue(sourceOriginal, liveEl, entry, binding);
        if (value === undefined) continue;
        values[entry.prop] = value;
        break;
      }
    }
    return values;
  ...
  function readSvelteBindingValue(sourceRoot, liveRoot, entry, binding) {
    const token = entry.placeholder || ('{' + entry.expr + '}');
    const sourceElements = [sourceRoot, ...sourceRoot.querySelectorAll('*')];

    if (binding?.kind === 'text') {
      for (const sourceEl of sourceElements) {
        const sourceTextNodes = [...sourceEl.childNodes].filter((node) => node.nodeType === Node.TEXT_NODE);
        let precedingTokenCount = 0;
        for (const sourceNode of sourceTextNodes) {
          const nodeTokens = (sourceNode.nodeValue || '').match(/\{[^{}]+\}/g) || [];
          if (nodeTokens[binding.tokenIndex] !== token
              || nodeTokens.length !== binding.tokenCount) {
            precedingTokenCount += nodeTokens.length;
            continue;
          }

          const liveEl = resolvePairedSvelteElement(sourceRoot, liveRoot, sourceEl);
          if (!liveEl) return undefined;
          const sourceValue = sourceTextNodes.map((node) => node.nodeValue || '').join('');
          const liveValue = [...liveEl.childNodes]
            .filter((node) => node.nodeType === Node.TEXT_NODE)
            .map((node) => node.nodeValue || '')
            .join('');
          return extractSvelteTemplateToken(
            normalizePreviewText(sourceValue),
            normalizePreviewText(liveValue),
            precedingTokenCount + binding.tokenIndex,
            true,
          );
        }
      }
    ...
    if (binding?.kind !== 'attribute' && binding?.kind !== 'directive') return undefined;
    const name = String(binding.name || '');
    if (!name) return undefined;
    for (const sourceEl of sourceElements) {
      const sourceAttr = sourceEl.getAttributeNode(name);
      if (!sourceAttr) continue;
      const tokens = sourceAttr.value.match(/\{[^{}]+\}/g) || [];
      if (tokens[binding.tokenIndex] !== token || tokens.length !== binding.tokenCount) continue;
      const liveEl = resolvePairedSvelteElement(sourceRoot, liveRoot, sourceEl);
      if (!liveEl) return undefined;
      if (binding.kind === 'directive') {
        return readSvelteDirectiveDomValue(liveEl, name);
      }

      const liveValue = liveEl.getAttribute(name);
      if (liveValue == null) {
        const propertyValue = liveEl[name];
        return typeof propertyValue === 'boolean' ? false : undefined;
      }
      if (typeof liveEl[name] === 'boolean' && sourceAttr.value.trim() === token) {
        return liveEl[name];
      }
      return extractSvelteTemplateToken(sourceAttr.value, liveValue, binding.tokenIndex, false);
    }
    return undefined;
  }

  function resolvePairedSvelteElement(sourceRoot, liveRoot, sourceElement) {
    if (sourceElement === sourceRoot) return liveRoot;
    const path = [];
    ...
    let liveElement = liveRoot;
    for (const index of path.reverse()) {
      liveElement = liveElement?.children?.[index] || null;
      if (!liveElement) return null;
    }
    return liveElement;
  }

  function extractSvelteTemplateToken(sourceValue, liveValue, tokenIndex, trimResult) {
    ...
    // Adjacent expressions (including whitespace-only separation) cannot be
    // recovered independently from one flattened DOM string.
    if (tokens.length > 1 && literals.slice(1, -1).some((literal) => !/\S/.test(literal))) {
      return undefined;
    }

    let pattern = '^';
    for (let index = 0; index < tokens.length; index++) {
      pattern += svelteTemplateLiteralPattern(literals[index], trimResult);
      pattern += '(.*?)';
    }
    pattern += svelteTemplateLiteralPattern(literals[literals.length - 1], trimResult) + '$';
    const match = live.match(new RegExp(pattern));
    if (!match) return undefined;
    const value = match[tokenIndex + 1];
    return trimResult ? value.trim() : value;
  }

  function svelteTemplateLiteralPattern(literal, flexibleWhitespace) {
    if (!flexibleWhitespace) return escapeRegExp(literal);
  ...
  function readSvelteDirectiveDomValue(liveEl, sourceName) {
    const colon = sourceName.indexOf(':');
    if (colon < 0) return undefined;
    const directive = sourceName.slice(0, colon);
    const argument = sourceName.slice(colon + 1).split('|')[0];
    if (!argument) return undefined;
    if (directive === 'class') return liveEl.classList.contains(argument);
    if (directive === 'style') return liveEl.style.getPropertyValue(argument);
    if (directive === 'bind' && argument in liveEl && argument !== 'this') return liveEl[argument];
    return undefined;
  }
