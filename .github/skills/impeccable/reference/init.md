...
Decision tree (apply the prerequisite gate first, then take exactly one file-state branch):

1. **Prerequisite gate for every existing PRODUCT.md:** before deciding what to do about DESIGN.md, refreshing either file, or configuring live mode, inspect PRODUCT.md for both required context fields.
   - If `## Register` is missing (legacy), use Step 2 to form a hypothesis, confirm it with the user in Step 3, then add the section.
   - If `## Platform` is missing (legacy), use Step 2 to form a hypothesis, confirm `web` / `ios` / `android` / `adaptive` with the user in Step 3, then add the section. Do this for every legacy PRODUCT.md, including web projects; absence is unknown, never an implicit `web`.
   - If both are missing, capture Register first and Platform immediately after it. Complete both repairs before continuing.
2. **Take exactly one branch from this exhaustive file-state matrix:**
   - **Neither PRODUCT.md nor DESIGN.md exists:** do Steps 2-4 to write PRODUCT.md (including Platform), then continue to Step 5.
   - **Only PRODUCT.md exists:** after the prerequisite gate is complete, continue to Step 5 and offer `/impeccable document` for DESIGN.md.
   - **Only DESIGN.md exists (unusual):** do Steps 2-4 to write PRODUCT.md (including Platform), then keep or refresh the existing DESIGN.md only with the user's confirmation.
   - **Both exist:** after the prerequisite gate is complete, ask which file, if either, the user wants refreshed. Ask the user directly about anything the selected file needs that cannot be inferred; skip every file they do not want changed.

Never silently overwrite an existing file. Always confirm first. Never proceed to Step 5 or Step 6 until PRODUCT.md contains a user-confirmed Platform.
...
**Platform gate:** enter this step only after PRODUCT.md contains a user-confirmed `## Platform`. A missing Platform is not `web`; return to Steps 2-4 to capture it before applying either the native skip or the web live-mode path.
