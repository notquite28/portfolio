...
2. If the user invoked a command from the Commands table (`craft`, `shape`, `audit`, `polish`, ...), you MUST read the command's reference next: **the exact Reference linked in the table, using its native variant when the project platform is native** (`ios` / `android` / `adaptive`, per the `context.mjs` directive). One file, not both. Non-optional. The reference defines the command's flow; without it you will skip steps the user expects. Management commands are routed separately: `pin` and `unpin` have no reference file and go directly to Pin / Unpin below; `hooks` loads only `reference/hooks.md`.
...
2. **First word matches a command in the table above**: load its linked reference file (on native platforms, the table's native variant; Setup step 2's one-file rule) and follow its instructions. Everything after the command name is the target.
3. **First word is `pin` or `unpin`**: do not look for a reference file. Run the bundled `pin.mjs` command exactly as specified in Pin / Unpin below, passing the first word as its action and the remaining word as its command.
4. **First word is `hooks`**: load [reference/hooks.md](reference/hooks.md) and follow it with everything after `hooks` as the management action and arguments.
5. **First word doesn't match, but the intent clearly maps to one command** (e.g. "fix the spacing" → `layout`, "rewrite this error message" → `clarify`, "the colors feel flat" → `colorize`): load that command's reference (same native-variant rule) and proceed as if invoked. If two commands could fit, ask once which.
6. **No clear command match**: general design invocation. Apply the setup steps, the General rules, and the loaded register reference, using the full argument as context.
